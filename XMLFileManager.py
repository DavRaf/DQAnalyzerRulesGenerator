...
class XMLFileManager:
    ...
    def read_rules_from_plan_file(self, file):
        with open(file, 'r', errors='ignore') as f:
            ...
            for rule in rules:
                ...
                if 'Data Range' not in name:
                    expression = html.unescape(expression).replace("\n", "").replace("\t", "")
                    generated_rule = GeneratedRule(column_name=expression.split('\"')[2], rule_name=name, rule_expression=expression)
                else:
                    generated_rule = GeneratedRule(column_name=expression.split(' ')[-1], rule_name=name, rule_expression=expression)
    ...
    def read_transformers(self):
        ...
        with open('corrections.xml', 'r', errors='ignore') as f:
            doc = minidom.parse(f)
    ...
    def write_rule_advanced(self, file, rule_name, rule_expression):
        with open(file, 'r', errors='ignore') as f:
            ...
            if 'matches' in rule_expression:
                element.setAttribute(XMLFileManager.NAME, rule_name  + " " + rule_expression.split("\"")[2].replace(", ", "").replace(")", ""))
            else:
